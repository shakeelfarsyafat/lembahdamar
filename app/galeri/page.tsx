...
import { Building2, Camera, ShieldCheck, Sparkles } from "lucide-react";
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#F7F5F0] font-sans">
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14 space-y-16 w-full">
        ...
        {/* SECTION 1: LOGO MITRA KERJA SAMA (Card kecil 5 baris/kolom) */}
        <section className="space-y-6">
          ...
          {partners.length === 0 ? (
            <div className="bg-white p-8 rounded-2xl border border-dashed border-stone-300 text-center text-stone-500 text-xs font-bold">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 sm:gap-5">
              {partners.map((partner, idx) => (
                <div
                  key={partner.id || idx}
                  className="bg-white p-4 rounded-2xl border border-stone-200/80 shadow-xs hover:shadow-md transition-all flex flex-col justify-between items-center text-center space-y-3 group hover:border-[#FF5524]/40"
                >
                  <div className="w-full h-20 bg-stone-50 rounded-xl overflow-hidden flex items-center justify-center p-3 border border-stone-100 group-hover:scale-105 transition-transform">
                    <img
                      src={partner.logoUrl}
                      alt={partner.name}
                  ...
                  <div>
                    <h3 className="font-extrabold text-stone-900 text-xs truncate max-w-[130px]">
                      {partner.name}
                    </h3>
          ...
          {galleryItems.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {galleryItems.map((item, idx) => (
                <div
                  key={item.id || idx}
                  className="bg-white rounded-2xl border border-stone-200/80 overflow-hidden shadow-xs hover:shadow-lg transition-all duration-300 flex flex-col justify-between group"
                >
                  <div className="relative h-56 w-full overflow-hidden bg-stone-100">
                    ...
                    />
                    <div className="absolute top-3 left-3 bg-black/80 text-white text-[10px] font-bold px-3 py-1 rounded-md backdrop-blur-xs">
